...
public class Withdraw_money_Client {
    ...
    private static String poundage;//手续费率
...
//新增top4动态获取
                    intent.putExtra("tip4",bankCard.getJSONObject(0).optString("redeemFeeTip"));//金额未满2天转出收取0.5%手续费
//1012添加
//                    intent.putExtra("free",list.optDouble("free"));//转出手续费
                    intent.putExtra("card", card);//银行卡信息
                    intent.putExtra("capitalBalance", list.optString("capitalBalance") + "");//余额

                    intent.putExtra("free",bankCard.getJSONObject(0).optDouble("poundage"));//手续费
                    LogUtils.e("返回的利率：", bankCard.getJSONObject(0).optDouble("poundage") + "");
