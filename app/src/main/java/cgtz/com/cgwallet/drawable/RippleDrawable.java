package cgtz.com.cgwallet.drawable;
...
import cgtz.com.cgwallet.master.ColorUtil;
import cgtz.com.cgwallet.master.ThemeUtil;
import cgtz.com.cgwallet.master.ViewUtil;
...
public class RippleDrawable extends Drawable implements Animatable,	OnTouchListener {
	...
	private RippleDrawable(Drawable backgroundDrawable, int backgroundAnimDuration, int backgroundColor, int rippleType, boolean delayClick, int maxRippleRadius, int rippleAnimDuration, int rippleColor, Interpolator inInterpolator, Interpolator outInterpolator, int type, int topLeftCornerRadius, int topRightCornerRadius, int bottomRightCornerRadius, int bottomLeftCornerRadius, int left, int top, int right, int bottom){
		...
		mInShader = new RadialGradient(0, 0, GRADIENT_RADIUS,
				new int[]{mRippleColor, mRippleColor, 0},
				GRADIENT_STOPS, Shader.TileMode.CLAMP);
		if(mRippleType == TYPE_WAVE)
			mOutShader = new RadialGradient(0, 0, GRADIENT_RADIUS,
					new int[]{0, ColorUtil.getColor(mRippleColor, 0f),
							mRippleColor}, GRADIENT_STOPS, Shader.TileMode.CLAMP);
	}
    ...
    public void setMask(int type, int topLeftCornerRadius,
						int topRightCornerRadius, int bottomRightCornerRadius,
						int bottomLeftCornerRadius, int left, int top, int right, int bottom){
        mMask = new Mask(type, topLeftCornerRadius, topRightCornerRadius,
				bottomRightCornerRadius, bottomLeftCornerRadius, left, top, right, bottom);
    }
	...
	public long getClickDelayTime(){
		if(mState == STATE_RELEASE_ON_HOLD)			
			return (mDelayClick ? 2 : 1) * Math.max(mBackgroundAnimDuration, mRippleAnimDuration)
					- (SystemClock.uptimeMillis() - mStartTime);
		else if(mState == STATE_RELEASE)
			return (mDelayClick ? Math.max(mBackgroundAnimDuration, mRippleAnimDuration) : 0)
					- (SystemClock.uptimeMillis() - mStartTime);
