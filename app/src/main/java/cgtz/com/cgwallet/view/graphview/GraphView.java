...
package cgtz.com.cgwallet.view.graphview;
...
import cgtz.com.cgwallet.utils.llutils.MyLog;
import cgtz.com.cgwallet.view.graphview.GraphViewSeries.GraphViewSeriesStyle;
import cgtz.com.cgwallet.view.graphview.GraphViewStyle.GridStyle;
 ...
 * {@link #drawSeries(Canvas, GraphViewDataInterface[], float, float, float, double, double, double, double, float, com.jjoe64.graphview.GraphViewSeries.GraphViewSeriesStyle)}
 * to display a custom graph. Use {@link com.jjoe64.graphview.LineGraphView} for creating a line
 * chart.
 * 
...
abstract public class GraphView extends LinearLayout {
  static final public class GraphViewConfig {
    // 标题 下边框 间距 单位dp   15最合适
    static final float BORDER = 33;
...
/**
    ...
    protected void onDraw(Canvas canvas) {
      ...
      for (int i = 0; i < graphSeries.size(); i++) {
        // 填充数值(表中橙色区域) 第十个参数是图表的左侧位移
       // 填充数值
        //  drawSeries(canvas, _values(i), graphwidth, graphheight, border, minX, minY, diffX, diffY,
        //   horstart + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2),graphSeries.get(i).style);
...
/**
    ...
    public VerLabelsView(Context context) {
      super(context);
      setLayoutParams(new LayoutParams(getGraphViewStyle().getVerticalLabelsWidth() == 0 ? 100 : getGraphViewStyle().getVerticalLabelsWidth(), LayoutParams.MATCH_PARENT));
    }
...
/**
    ...
    protected void onDraw(Canvas canvas) {
      ...
      if (getGraphViewStyle().getVerticalLabelsWidth() == 0 && getLayoutParams().width != verLabelTextWidth + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2)) {
        setLayoutParams(new LayoutParams(verLabelTextWidth + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2), LayoutParams.FILL_PARENT));
      ...
      // vertical labels
      paint.setTextAlign(getGraphViewStyle().getVerticalLabelsAlign());
      //这是左侧标签距离图表的宽度   int labelsWidth = getWidth();
      int labelsWidth = getWidth();
      int labelsOffset = 0;
      if (getGraphViewStyle().getVerticalLabelsAlign() == Align.RIGHT) {
        labelsOffset = labelsWidth;
      ...
      for (int i = 0; i < verlabels.length; i++) {
        ...
        */
/*if(i%2 == 0){
          canvas.drawText("1", labelsOffset, y + DensityUtil.dip2px(context, 5), paint);
        }else{
          canvas.drawText(verlabels[i], labelsOffset-5, y + DensityUtil.dip2px(context, 5), paint);//原
        }*//*

        if(i == 1){
          canvas.drawText("10%", labelsOffset-5, y + DensityUtil.dip2px(context, 5), paint);
        }else if(i == 3){
          canvas.drawText("8.5%", labelsOffset-5, y + DensityUtil.dip2px(context, 5), paint);//原
        }else if (i==5){
          canvas.drawText("7%", labelsOffset-5, y + DensityUtil.dip2px(context, 5), paint);
        }
      }
...
/**
  ...
  protected void drawHorizontalLabels(Canvas canvas, float border, float horstart, float height, String[] horlabels, float graphwidth) {
    ...
    for (int i = 0; i < horlabels.length; i++) {
      paint.setColor(graphViewStyle.getGridColor());
      //horstart就是左边的突出的横线的长度     float x = ((graphwidth / hors) * i) + horstart;
      float x = ((graphwidth / hors) * i)+ horstart;
      if (graphViewStyle.getGridStyle() != GridStyle.VERTICAL) {
        canvas.drawLine(x, height - border, x, border, paint);
      }
      if (showHorizontalLabels) {
        paint.setTextAlign(Align.CENTER);
        paint.setColor(graphViewStyle.getHorizontalLabelsColor());
        // 绘制底部标签文字  第三个参数是下部标签和空白一起的高度    canvas.drawText(horlabels[i], x, height - DensityUtil.dip2px(context, 5), paint);
